...
def use_cli(request, monkeypatch):
    """Point Taguchi at the known CLI path.

    Opt out with @pytest.mark.real_discovery where discovery itself is what is
    being tested and has to be allowed to fail.
    """
    if "real_discovery" in request.keywords:
        return
...
# ---- merged from test_integration_enhanced.py ----
import asyncio
import os
import tempfile
import pytest
