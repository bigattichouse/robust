...
def use_cli(monkeypatch):
    ...
    monkeypatch.setattr(core.Taguchi, "_find_cli", lambda self: CLI_PATH)
    ...
    def test_cleans_up_on_exception(self, two_factor_exp):
        ...
        try:
            with Analyzer(two_factor_exp) as a:
                # Full results: main_effects() refuses an incomplete set.
                for i, run in enumerate(two_factor_exp.generate(), start=1):
                    a.add_result(i, float(i))
                a.main_effects()
                saved_path = a._csv_path
...
# ---- merged from test_analyzer_enhanced.py ----
import csv
import tempfile
import pytest
...
class TestAnalyzerEnhanced:
    ...
    def test_init_default(self, mock_experiment, mock_taguchi):
        ...
        with patch('taguchi.analyzer.Taguchi', return_value=mock_taguchi):
    ...
    def test_init_with_config(self, mock_experiment):
        ...
        with patch('taguchi.analyzer.Taguchi') as mock_taguchi_class:
